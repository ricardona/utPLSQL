create or replace package body ut_dbms_output_reporter as
  c_dashed_line constant varchar2(80) := '--------------------------------------------------------------------------------';

  procedure begin_suite(a_suite in ut_test_suite) as
  begin
    dbms_output.put_line(c_dashed_line);
    dbms_output.put_line('suite "' || nvl(a_suite.suite_name, '') || '" started.');
  end;

  procedure end_suite(a_suite in ut_test_suite, a_results in ut_suite_results) as
  begin
  ...
  procedure begin_test(a_test in ut_single_test, a_in_suite in boolean) as
  begin
    null;
  end;
