create or replace type body ut_test_call_params
is
    static
procedure execute_call (
        a_owner      varchar2 ,
        a_object     varchar2 ,
        a_subprogram varchar2 )
is
    stmt varchar2 ( 200 ) ;
    i    number;
    c    number;
begin
    ...
    $if $$ut_trace $then dbms_output.put_line ( 'ut_test_call_params.execute_call stmt:' || stmt ) ;
    $end i := dbms_sql.open_cursor;
...
*/
member procedure validate_params (
        self in ut_test_call_params ,
        a_result out boolean )
is
begin
    a_result            := true;
    ...
    if a_result and not ut_metadata.package_valid ( self.owner_name ,self.object_name ) then
        a_result := false;
        ut_assert.report_error ( 'package does not exist or is invalid: ' || nvl ( self.object_name ,'<missing package name>' ) ) ;
    end if;
    if a_result then
        if not ut_metadata.procedure_exists ( self.owner_name ,self.object_name ,self.test_procedure ) then
            ut_assert.report_error ( 'package missing test method ' || self.object_name || '.' || nvl ( self.test_procedure ,'<missing procedure name>' ) ) ;
        end if;
        if self.setup_procedure is not null and not ut_metadata.procedure_exists ( self.owner_name ,self.object_name ,self.setup_procedure ) then
            ut_assert.report_error ( 'package missing setup method ' || self.object_name || '.' || nvl ( self.setup_procedure ,'<missing procedure name>' ) ) ;
        end if;
        if self.teardown_procedure is not null and not ut_metadata.procedure_exists ( self.owner_name ,self.object_name ,self.teardown_procedure ) then
            ut_assert.report_error ( 'package missing teardown method ' || self.object_name || '.' || nvl ( self.teardown_procedure ,'<missing procedure name>' ) ) ;
        end if;
    end if;
end validate_params;
member procedure setup (
        self in ut_test_call_params )
is
begin
