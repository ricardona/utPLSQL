create or replace type body ut_test
is
    constructor
function ut_test (
        a_object_name    varchar2 ,
        a_test_procedure varchar2 ,
        a_test_name in varchar2 default null ,
        a_owner_name         varchar2 default null ,
        a_setup_procedure    varchar2 default null ,
        a_teardown_procedure varchar2 default null )
    return self
as
    result
is
begin
    ...
    self.call_params := ut_test_call_params ( object_name => trim ( a_object_name ) ,test_procedure => trim ( a_test_procedure ) ,owner_name => coalesce ( trim ( a_owner_name ) ,sys_context ( 'userenv' ,'current_schema' ) ) ,setup_procedure => trim (
    a_setup_procedure ) ,teardown_procedure => trim ( a_teardown_procedure ) ) ;
    return;
end ut_test;
member function is_valid (
        self in ut_test )
    return boolean
is
begin
    ...
    if call_params.setup_procedure is not null and not ut_metadata.do_resolve ( call_params.owner_name ,call_params.object_name ,call_params.setup_procedure ) then
        return false;
    end if;
    if call_params.teardown_procedure is not null and not ut_metadata.do_resolve ( call_params.owner_name ,call_params.object_name ,call_params.teardown_procedure ) then
        return false;
    end if;
    return true;
end is_valid;
member function setup_stmt (
        self in ut_test )
    return varchar2
is
begin
...
overriding member procedure execute (
        self       in out nocopy ut_test ,
        a_reporter in out nocopy ut_suite_reporter )
is
...
begin
    ...
    begin
        $if $$ut_trace $then dbms_output.put_line ( 'ut_test.execute' ) ;
        $end self.execution_result := ut_execution_result ( ) ;
        ...
        if params_valid then
            ...
            when others then
                ...
                $if $$ut_trace $then dbms_output.put_line ( 'testmethod failed-' || sqlerrm ( sqlcode ) || ' ' || dbms_utility.format_error_backtrace ) ;
                $end ut_assert.report_error ( sqlerrm ( sqlcode ) || ' ' || dbms_utility.format_error_backtrace ) ;
            end;
    ...
    exception
    when others then
        $if $$ut_trace $then dbms_output.put_line ( 'ut_test.execute failed-' || sqlerrm ( sqlcode ) || ' ' || dbms_utility.format_error_backtrace ) ;
        $end
    ...
    if a_reporter is not null then
        a_reporter.end_test ( a_test_name => self.name ,a_test_call_params => self.call_params ,a_execution_result => self.execution_result ,a_assert_list => self.assert_results ) ;
    end if;
end;
overriding member procedure execute (
        self in out nocopy ut_test )
